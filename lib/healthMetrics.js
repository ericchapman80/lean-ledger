...
import { DAY_TYPE_OPTIONS } from '@/lib/coachingProfile.js';
import {
  cmToInches,
  flOzToMl,
  inchesToCm,
...
export const HEALTH_METRIC_FIELDS = [
  ...
  { key: 'dayType', label: 'Day Type', unit: '', importable: false, core: true, type: 'text' },
  { key: 'readingCompleted', label: 'Reading Completed', unit: '', importable: false, core: true, type: 'boolean' },
  { key: 'prayerCompleted', label: 'Prayer Completed', unit: '', importable: false, core: true, type: 'boolean' },
  {
    key: 'hydrationOunces',
    label: 'Hydration',
    unit: 'oz',
...
export function validateHealthMetricEntry(input, options = {}) {
  ...
  if (normalized.dayType != null && !DAY_TYPE_OPTIONS.includes(normalized.dayType)) {
    errors.push('Day Type must be one of the supported day contexts');
  }
