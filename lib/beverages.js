...
const ALWAYS_COUNTS_TOWARD_HYDRATION_TYPES = new Set([
  'water',
  'sparkling_water',
  'electrolyte_drink',
  'black_coffee',
...
export const BEVERAGE_TYPES = [
  ...
  { key: 'black_coffee', label: 'Black Coffee', countsTowardHydration: true, hydrationMultiplier: 0.9, defaults: { calories: 0, protein: 0, carbs: 0, fat: 0 } },
...
export function getDefaultCountsTowardHydration(beverageType) {
  return Boolean(getBeverageTypeMeta(beverageType).countsTowardHydration);
}

export function getHydrationBeverageType(entry = {}) {
  const beverageType = typeof entry === 'string' ? entry : entry.beverageType;
  if (beverageType && beverageType !== 'other') return beverageType;

  const name = normalizeName(typeof entry === 'string' ? '' : entry.displayName || entry.name);
  if (/\b(americano|espresso|coffee|cold brew)\b/.test(name)) return 'black_coffee';
  if (/\b(tea|iced tea)\b/.test(name)) return 'unsweet_tea';
  if (/\b(electrolyte|lmnt|liquid i\.?v\.?|nuun)\b/.test(name)) return 'electrolyte_drink';
  if (/\b(protein|shake)\b/.test(name)) return 'protein_shake';
  if (/\bmilk\b/.test(name)) return 'milk';

  return beverageType || 'water';
}

export function normalizeCountsTowardHydration({ beverageType = 'water', displayName = null, countsTowardHydration } = {}) {
  const effectiveType = getHydrationBeverageType({ beverageType, displayName });
  if (ALWAYS_COUNTS_TOWARD_HYDRATION_TYPES.has(effectiveType)) return true;

  const explicitValue = booleanOrNull(countsTowardHydration);
  if (explicitValue != null) return explicitValue;
  if (beverageType === 'other' && effectiveType !== 'other') return true;
  return getDefaultCountsTowardHydration(beverageType);
}

export function shouldCountTowardHydration(entry = {}) {
  return normalizeCountsTowardHydration({
    beverageType: entry.beverageType,
    displayName: entry.displayName || entry.name,
    countsTowardHydration: entry.countsTowardHydration,
  });
}

export function shouldCountTowardHydration(entry = {}) {
  return Boolean(entry?.countsTowardHydration);
}
...
export function getHydrationContributionFlOz(entry) {
  if (!shouldCountTowardHydration(entry)) return 0;
  const multiplier = getHydrationMultiplier(getHydrationBeverageType(entry));
  return round(Number(entry.amountFlOz || 0) * multiplier, 1);
}

export function getHydrationContributionLabel(entry, preferredUnit = 'fl_oz') {
  const contributionFlOz = getHydrationContributionFlOz(entry);
  if (contributionFlOz <= 0) return 'does not add to hydration total';
  return `${formatBeverageFromFlOz(contributionFlOz, preferredUnit)} hydration credit`;
}
...
export function normalizeBeverageEntryInput({
  ...
  const normalizedCountsTowardHydration = normalizeCountsTowardHydration({
    beverageType,
  ...
  return {
    ...
    countsTowardHydration: normalizedCountsTowardHydration,
...
export function summarizeBeverageEntries(entries, {
  ...
  return {
    ...
    hydrationDetails: entries.map((entry) => {
      const countsTowardHydration = shouldCountTowardHydration(entry);
      const hydrationBeverageType = getHydrationBeverageType(entry);
      return {
        beverageType: entry.beverageType || 'water',
        hydrationBeverageType,
        amountFlOz: Number(entry.amountFlOz || 0),
        hydrationMultiplier: countsTowardHydration ? getHydrationMultiplier(hydrationBeverageType) : 0,
