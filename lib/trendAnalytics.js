...
import { mergeDailyWinDefinitions, isDailyWinComplete } from '@/lib/dailyWins.js';
import { buildDailyWinChallengeSummary, findDailyWinTemplate } from '@/lib/dailyWinTemplates.js';
...
function buildRecoveryBehaviorSummary(dailySeries) {
  const workoutEntries = dailySeries.filter((entry) => entry.workoutCompleted != null);
  const workoutDays = workoutEntries.filter((entry) => entry.workoutCompleted);
  const restDays = workoutEntries.filter((entry) => entry.workoutCompleted === false);
  const sleepEntries = dailySeries.filter((entry) => Number.isFinite(entry.sleepHours));
  const lowSleepEntries = sleepEntries.filter((entry) => entry.sleepHours < 6.5);
  const adequateSleepEntries = sleepEntries.filter((entry) => entry.sleepHours >= 6.5);
  const proteinWorkoutAverage = workoutDays.length > 0
    ? round(average(workoutDays.map((entry) => Number(entry.protein || 0))))
    : null;
  const proteinRestAverage = restDays.length > 0
    ? round(average(restDays.map((entry) => Number(entry.protein || 0))))
    : null;
  const hydrationWorkoutHitRate = workoutDays.length > 0
    ? toPercentage((workoutDays.filter((entry) => Number(entry.hydrationOunces || 0) >= Number(entry.hydrationTarget || 0)).length / workoutDays.length) * 100)
    : null;
  const hydrationRestHitRate = restDays.length > 0
    ? toPercentage((restDays.filter((entry) => Number(entry.hydrationOunces || 0) >= Number(entry.hydrationTarget || 0)).length / restDays.length) * 100)
    : null;
  const averageSleepHours = sleepEntries.length > 0
    ? Number(average(sleepEntries.map((entry) => entry.sleepHours)).toFixed(1))
    : null;
  const workoutSleepAverage = workoutDays.filter((entry) => Number.isFinite(entry.sleepHours)).length > 0
    ? Number(average(workoutDays.filter((entry) => Number.isFinite(entry.sleepHours)).map((entry) => entry.sleepHours)).toFixed(1))
    : null;
  const restSleepAverage = restDays.filter((entry) => Number.isFinite(entry.sleepHours)).length > 0
    ? Number(average(restDays.filter((entry) => Number.isFinite(entry.sleepHours)).map((entry) => entry.sleepHours)).toFixed(1))
    : null;
  const lowSleepEnergyAverage = lowSleepEntries.filter((entry) => Number.isFinite(entry.energyLevel)).length > 0
    ? Number(average(lowSleepEntries.filter((entry) => Number.isFinite(entry.energyLevel)).map((entry) => entry.energyLevel)).toFixed(1))
    : null;
  const adequateSleepEnergyAverage = adequateSleepEntries.filter((entry) => Number.isFinite(entry.energyLevel)).length > 0
    ? Number(average(adequateSleepEntries.filter((entry) => Number.isFinite(entry.energyLevel)).map((entry) => entry.energyLevel)).toFixed(1))
    : null;
  const recoveryReadyEntries = dailySeries.filter((entry) => (
    Number.isFinite(entry.sleepHours)
    && Number.isFinite(entry.energyLevel)
    && Number.isFinite(entry.sorenessLevel)
  ));
  const recoveryReadyDays = recoveryReadyEntries.filter((entry) => (
    entry.sleepHours >= 7
    && entry.energyLevel >= 4
    && entry.sorenessLevel <= 3
  )).length;
  ...
  workoutEntries.forEach((entry) => {
    if (entry.workoutCompleted) {
      runningStreak += 1;
      longestWorkoutStreak = Math.max(longestWorkoutStreak, runningStreak);
    } else {
      runningStreak = 0;
    }
  });
  for (let index = workoutEntries.length - 1; index >= 0; index -= 1) {
    if (!workoutEntries[index].workoutCompleted) break;
    currentWorkoutStreak += 1;
  }

  return {
    loggedWorkoutDays: workoutEntries.length,
    ...
    workoutSleepAverage,
    restSleepAverage,
    lowSleepDays: lowSleepEntries.length,
    proteinWorkoutAverage,
    proteinRestAverage,
    hydrationWorkoutHitRate,
    hydrationRestHitRate,
    recoveryReadyDays,
    recoveryReadyLoggedDays: recoveryReadyEntries.length,
    lowSleepEnergyAverage,
    adequateSleepEnergyAverage,
  };
}

function buildDailyWinsBehaviorSummary(dailySeries, activeDailyWins) {
  const definitions = activeDailyWins || [];
  const trackedDays = dailySeries.filter((entry) => (
  ...
  const completionCounts = trackedDays.map((entry) => (
    definitions.filter((definition) => isDailyWinComplete(definition, entry)).length
  ));

  const totalPossibleWins = totalTrackedDays * definitions.length;
  const totalCompletedWins = completionCounts.reduce((sum, value) => sum + value, 0);
  const habitCompletionPercentages = Object.fromEntries(
    definitions.map((definition) => [
      definition.key,
      totalTrackedDays > 0
        ? toPercentage((trackedDays.filter((entry) => isDailyWinComplete(definition, entry)).length / totalTrackedDays) * 100)
        : null,
    ]),
  );

  return {
    activeTotal: definitions.length,
    activeKeys: definitions.map((definition) => definition.key),
    averageCompletedWins: totalTrackedDays > 0 ? Number((average(completionCounts) || 0).toFixed(1)) : null,
    completionPercentage: totalPossibleWins > 0 ? toPercentage((totalCompletedWins / totalPossibleWins) * 100) : null,
    perfectDays: completionCounts.filter((value) => value === definitions.length).length,
    habitCompletionPercentages,
  };
}

function buildDailyWinsChallengeBehaviorSummary(dailySeries, activeDailyWins, profile, endDate) {
  const challengeSummary = buildDailyWinChallengeSummary({
  ...
  const challengeEntries = dailySeries.filter((entry) => entry.date >= profile.dailyWinsChallengeStartDate);
  if (challengeEntries.length === 0) {
    return {
      ...challengeSummary,
      trackedDaysInRange: 0,
      perfectDaysInRange: 0,
      currentPerfectDayStreak: 0,
      longestPerfectDayStreak: 0,
      completionPercentageInRange: null,
    };
  }

  const completionCounts = challengeEntries.map((entry) => (
    activeDailyWins.filter((definition) => isDailyWinComplete(definition, entry)).length
  ));
  const perfectFlags = completionCounts.map((count) => count === activeDailyWins.length);
  let currentPerfectDayStreak = 0;
  let longestPerfectDayStreak = 0;
  let runningStreak = 0;

  perfectFlags.forEach((isPerfect) => {
    if (isPerfect) {
      runningStreak += 1;
      longestPerfectDayStreak = Math.max(longestPerfectDayStreak, runningStreak);
    } else {
      runningStreak = 0;
    }
  });

  for (let index = perfectFlags.length - 1; index >= 0; index -= 1) {
    if (!perfectFlags[index]) break;
    currentPerfectDayStreak += 1;
  }

  const totalPossibleWins = challengeEntries.length * activeDailyWins.length;
  const totalCompletedWins = completionCounts.reduce((sum, value) => sum + value, 0);
  const template = findDailyWinTemplate(profile.dailyWinsTemplateKey);

  return {
    ...challengeSummary,
    templateDescription: template?.description ?? null,
    trackedDaysInRange: challengeEntries.length,
    perfectDaysInRange: perfectFlags.filter(Boolean).length,
    currentPerfectDayStreak,
    longestPerfectDayStreak,
    completionPercentageInRange: totalPossibleWins > 0
      ? toPercentage((totalCompletedWins / totalPossibleWins) * 100)
      : null,
  };
}
...
export function buildTrendAnalytics({
  ...
  const dailyHabitLogMap = dailyHabitLogs.reduce((acc, entry) => {
    if (!acc[entry.date]) acc[entry.date] = {};
    acc[entry.date][`habit:${entry.habitId}`] = entry.completed;
    return acc;
  }, {});
  ...
  const baseHydrationTarget = calculateDailyWaterTarget(weightLogs[0]?.weight ?? profile.weight, {
  ...
  let dailySeries = visibleDates.map((date, visibleIndex) => {
    ...
    const customHabitValues = dailyHabitLogMap[date] || {};
    ...
    return {
      ...
      readingCompleted: healthEntry.readingCompleted ?? null,
      prayerCompleted: healthEntry.prayerCompleted ?? null,
      ...customHabitValues,
      hydrationOunces,
      totalFluidsOunces,
  ...
  const recoveryBehavior = buildRecoveryBehaviorSummary(dailySeries);
  const activeDailyWins = mergeDailyWinDefinitions(profile.dailyWinsActiveKeys, customHabits);
  const dailyWinsBehavior = buildDailyWinsBehaviorSummary(
    dailySeries,
    activeDailyWins,
  );
  const dailyWinsChallengeBehavior = buildDailyWinsChallengeBehaviorSummary(
    dailySeries,
    activeDailyWins,
    profile,
    endDate,
