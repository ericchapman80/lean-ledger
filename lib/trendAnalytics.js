...
function buildBeverageBehaviorSummary(dailySeries, beverageEntries) {
  ...
  const beverageMixByType = beverageEntries.reduce((acc, entry) => {
    ...
    if (!acc[beverageType]) {
      acc[beverageType] = {
        beverageType,
        label: getBeverageTypeMeta(beverageType).label,
        totalFluidsOunces: 0,
        hydrationOunces: 0,
        entriesCount: 0,
      };
    ...
    acc[beverageType].totalFluidsOunces = round(acc[beverageType].totalFluidsOunces + Number(entry.amountFlOz || 0), 1);
    acc[beverageType].hydrationOunces = round(acc[beverageType].hydrationOunces + getHydrationContributionFlOz(entry), 1);
    acc[beverageType].entriesCount += 1;
    return acc;
  }, {});
