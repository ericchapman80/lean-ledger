...
function buildDailyWinsBehaviorSummary(dailySeries) {
  const trackedDays = dailySeries.filter((entry) => (
    entry.workoutCompleted != null
    || entry.readingCompleted != null
    || entry.prayerCompleted != null
    || Number.isFinite(entry.sleepHours)
    || Number.isFinite(entry.energyLevel)
    || Number.isFinite(entry.sorenessLevel)
  ));
  ...
  const completionCounts = trackedDays.map((entry) => (
    [
      ...
      entry.prayerCompleted === true,
      Number.isFinite(entry.sleepHours) && entry.sleepHours > 0,
      Number.isFinite(entry.energyLevel),
      Number.isFinite(entry.sorenessLevel),
    ].filter(Boolean).length
  ));

  const totalPossibleWins = totalTrackedDays * 6;
  const totalCompletedWins = completionCounts.reduce((sum, value) => sum + value, 0);

  const workoutCompletionPercentage = totalTrackedDays > 0
    ? toPercentage((trackedDays.filter((entry) => entry.workoutCompleted === true).length / totalTrackedDays) * 100)
    : null;
  const readingCompletionPercentage = totalTrackedDays > 0
    ? toPercentage((trackedDays.filter((entry) => entry.readingCompleted === true).length / totalTrackedDays) * 100)
    : null;
  const prayerCompletionPercentage = totalTrackedDays > 0
    ? toPercentage((trackedDays.filter((entry) => entry.prayerCompleted === true).length / totalTrackedDays) * 100)
    : null;
  ...
  return {
    ...
    perfectDays: completionCounts.filter((value) => value === 6).length,
...
export function buildTrendAnalytics({
  ...
  const dailyWinsBehavior = buildDailyWinsBehaviorSummary(dailySeries);
