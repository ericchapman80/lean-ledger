...
export async function create(data) {
  const rows = await sql`
    INSERT INTO favorite_meals (user_id, name, meal_type, protein, fat, carbs, calories)
    VALUES (${data.userId}, ${data.name}, ${data.mealType}, ${data.protein}, ${data.fat}, ${data.carbs}, ${data.calories})
    RETURNING *
  `;
  ...
  for (const item of data.items) {
    await sql`
      ...
      VALUES (
        ...
        ${item.protein}, ${item.fat}, ${item.carbs}, ${item.calories}
      )
    `;
  }
