...
export function enrichProfile(user) {
  ...
  const coachingMode = deriveCoachingMode({ ageGroup, goalStrategy: safeGoalStrategy, activityFocus });
  const allowedGoalStrategies = getAllowedGoalStrategies({ ageGroup, activityFocus });
  const youthSafetyMessage = getYouthSafetyMessage({ ageGroup, coachingMode });
  const coachingGuidance = getDayTypeGuidance({ dayType: user.dayType, coachingMode, ageGroup });
  ...
  const recommendedMacros = getRecommendedMacros(
    ...
    user.gender,
    user.activityLevel,
    getSafeMacroGoal({ ageGroup, goalStrategy: safeGoalStrategy, activityFocus }),
    user.dietStyle,
  );
...
export function validateProfilePayload(body) {
  ...
  const normalizedActivityFocus = normalizeActivityFocus(activityFocus);
  const ageGroup = deriveAgeGroup({ dateOfBirth, age: derivedAge });
  const safeGoalStrategy = normalizeGoalStrategyForAge({ ageGroup, goalStrategy, activityFocus: normalizedActivityFocus });
  const allowedGoalStrategies = getAllowedGoalStrategies({ ageGroup, activityFocus: normalizedActivityFocus });
  ...
  if (goalStrategy && safeGoalStrategy !== goalStrategy) return 'Goal strategy is not available for this age group';
