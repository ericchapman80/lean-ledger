  ...
  return ADVANCED_METRIC_GROUPS
    .map((group) => {
      ...
      return {
        ...
        rows,
        series,
        axisGroups: group.axisGroups
          ? {
              left: group.axisGroups.left.filter((field) => fields.includes(field)),
              right: group.axisGroups.right.filter((field) => fields.includes(field)),
            }
          : null,
      };
    })
