...
import { calculateNetCarbs } from '@/lib/carbUtils.js';
...
export function getCommonRestaurantItems(query) {
  ...
  return commonFoods
    ...
    .map((food) => {
      ...
      const sugarAlcohols = food.sugarAlcohols || 0;
      return {
        ...food,
        source: 'Common Foods',
        servingSize: 1,
        ...
        fiberPer100g: parseFloat((fiber / multiplier).toFixed(1)),
        sugarAlcoholsPer100g: parseFloat((sugarAlcohols / multiplier).toFixed(1)),
        fatPer100g: parseFloat((food.fat / multiplier).toFixed(1)),
        caloriesPer100g: Math.round(food.calories / multiplier),
        fiber,
        sugarAlcohols,
        netCarbs: calculateNetCarbs(food.carbs, fiber, sugarAlcohols),
      };
    });
}
...
export function parseProductData(product) {
  ...
  const sugarAlcoholsPer100g = nutriments['sugar-alcohol_100g'] || nutriments['sugar-alcohol'] || 0;
  ...
  return {
    ...
    fiber: parseFloat((fiberPer100g * multiplier).toFixed(1)),
    sugarAlcohols: parseFloat((sugarAlcoholsPer100g * multiplier).toFixed(1)),
    netCarbs: calculateNetCarbs(carbsPer100g * multiplier, fiberPer100g * multiplier, sugarAlcoholsPer100g * multiplier),
    ...
    fiberPer100g: parseFloat(Number(fiberPer100g).toFixed(1)),
    sugarAlcoholsPer100g: parseFloat(Number(sugarAlcoholsPer100g).toFixed(1)),
 ...
 */
export function parseUSDAFood(food) {
  ...
  const getNutrientByName = (matcher) => {
    const nutrient = nutrients.find((entry) => matcher(String(entry.nutrientName || entry.name || '')));
    return nutrient ? parseFloat(nutrient.value) : 0;
  };
  ...
  const sugarAlcohols = getNutrientByName((name) => name.toLowerCase().includes('sugar alcohol'));
  ...
  return {
    ...
    sugarAlcohols: parseFloat(sugarAlcohols.toFixed(1)),
    ...
    fiberPer100g: parseFloat(fiber.toFixed(1)),
    sugarAlcoholsPer100g: parseFloat(sugarAlcohols.toFixed(1)),
...
export function calculatePortionMacros(productData, portionGrams) {
  ...
  return {
    ...
    carbs: result.carbs,
    fiber: result.fiber,
    sugarAlcohols: result.sugarAlcohols,
