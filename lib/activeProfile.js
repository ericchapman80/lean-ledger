...
// Resolve the coaching subject (the source of targets, guardrails, diet style,
// daily wins) for the active profile. For the primary profile this is the
// authoritative users row (single source of truth for the account holder); for
// a dependent it's the profiles row. Callers pass `subject` to enrichProfile so
// a switched-to child sees its own (youth-safe) numbers instead of the parent's.
export async function resolveActiveCoachingSubject(request) {
  const userId = await getCurrentUserId(request);
  const activeProfileId = await getActiveProfileId(request);
  const primary = await Profile.findPrimaryByUserId(userId);

  if (!activeProfileId || (primary && activeProfileId === primary.id)) {
    const user = await User.findById(userId);
    return { subject: user, isPrimary: true, userId, profileId: primary?.id ?? null, householdId: null };
  }

  const profile = await Profile.findById(activeProfileId);
  return {
    subject: profile,
    isPrimary: false,
    userId,
