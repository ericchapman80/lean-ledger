...
export const performanceMetricsApi = {
  getPerformanceMetrics: (params = {}) => {
    const query = new URLSearchParams(params).toString();
    return request(`/performance-metrics${query ? `?${query}` : ''}`);
  },
  createPerformanceMetric: (data) =>
    request('/performance-metrics', { method: 'POST', body: JSON.stringify(data) }),
  deletePerformanceMetric: (id) => request(`/performance-metrics/${id}`, { method: 'DELETE' }),
};
...
export const bodyCompositionGoalsApi = {
  getGoals: () => request('/body-composition-goals'),
  createGoal: (data) => request('/body-composition-goals', { method: 'POST', body: JSON.stringify(data) }),
  updateGoal: (id, data) => request(`/body-composition-goals/${id}`, { method: 'PATCH', body: JSON.stringify(data) }),
  completeGoal: (id) => request(`/body-composition-goals/${id}/complete`, { method: 'POST' }),
  archiveGoal: (id) => request(`/body-composition-goals/${id}/archive`, { method: 'POST' }),
};
...
export const profilesApi = {
  ...
  unlinkProfile: (id) => request(`/profiles/${id}/unlink`, { method: 'POST' }),
};

export const householdLinksApi = {
  getInvitations: () => request('/household-links'),
  lookupExistingAccount: (email) => request(`/household-links/lookup?email=${encodeURIComponent(email)}`),
  inviteExistingAccount: (data) => request('/household-links', { method: 'POST', body: JSON.stringify(data) }),
  acceptInvitation: (id) => request(`/household-links/${id}/accept`, { method: 'POST' }),
  declineInvitation: (id) => request(`/household-links/${id}/decline`, { method: 'POST' }),
  revokeInvitation: (id) => request(`/household-links/${id}`, { method: 'DELETE' }),
};
