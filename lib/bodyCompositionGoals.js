...
function deriveStatus(goal, progress) {
  ...
  if (warnings.some((warning) => warning.includes('below'))) overall = BODY_COMPOSITION_STATUS.RED;
  else if (warnings.length > 0) overall = BODY_COMPOSITION_STATUS.YELLOW;
...
export function decorateGoalWithProgress(goal, currentSnapshot, weightEntries = []) {
  const progress = calculateBodyCompositionProgress(goal, currentSnapshot);
  const status = deriveStatus(goal, progress);
  const estimatedCompletionDate = estimateCompletionDate(goal, weightEntries);
